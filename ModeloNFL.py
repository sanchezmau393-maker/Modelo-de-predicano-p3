...
def cargar_datos_completos(anio):
    ...
    # CORRECCIÓN DEL ERROR: 'gameday' en lugar de 'game_date'
    rest_dict = {}
...
with tab1:
    ...
    if st.button("🚀 Ejecutar Modelo de Simulación", type="primary"):
        if eq_local == eq_vis:
            ...
            # Penalización Analítica por Lesión (70% de la Desviación Estándar del equipo)
            ...
            if qb_local_out or qb_vis_out:
                st.info(f"**Ajuste por Lesiones Aplicado:** El modelo detectó QBs suplentes y restó automáticamente {penalidad_calculada_local:.1f} pts a {eq_local} y {penalidad_calculada_vis:.1f} pts a {eq_vis} basado en su volatilidad ofensiva.")
